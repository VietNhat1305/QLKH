  ...
  methods: {
    ...
    async handleDelete() {
      if (this.model.id != 0 && this.model.id != null && this.model.id) {
        await this.$store.dispatch("theDiemStore/delete",{id :  this.model.id}).then((res) => {
          if (res.code===0) {
            this.fnGetList();
    ...
    async handleSubmit(e) {
      ...
      if (this.$v.$invalid) {
        ...
        if (
          ...
          // Update model
          await this.$store.dispatch("theDiemStore/update", this.model).then((res) => {
            if (res.code === 0) {
          ...
          await this.$store.dispatch("theDiemStore/create", this.model).then((res) => {
            if (res.code === 0) {
              this.fnGetList();
              this.showModal = false;
              this.model={}
    ...
    async handleUpdate(id) {
      await this.$store.dispatch("theDiemStore/getById", {id : id}).then((res) => {
        if (res.code===0) {
          console.log(res)
    ...
    myProvider (ctx) {
      ...
      this.loading = true
      try {
        let promise =  this.$store.dispatch("theDiemStore/getPagingParams", params)
